...
from ..modules.autoregressor import AutoRegressorModule
...
class AutoRegressivePipeline(Pipeline):
    ...
    def predict_next_token(
        ...
        with torch.no_grad():
            last_pred_logits: torch.Tensor = self._ar_predictor.model.get_last_logits(
                index_sequence
            )  # type: ignore  # BS x L x num_CB # TODO: check if correct
    ...
    def create_fixed_music_slice(
        ...
        end_index = self._ar_predictor.model.vocabulary_size + 1  # type: ignore # TODO: check if correct
